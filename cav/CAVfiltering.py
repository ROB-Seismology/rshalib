...
def calc_ln_CAV2(pga, M, vs30):
	...
	Parameters:
		pga: peak horizontal acceleration in g
		M: moment magnitude
		vs30: shear-wave velocity over the top 30 m in m/s
	Return value:
	...
	if len(pga) > 1:
		...
		if pga[0] <= 1.0:
			indexes_below = np.indices(ln_CAV.shape)
			indexes_above = np.array([], dtype='i')
		else:
			indexes_below = np.array([], dtype='i')
			indexes_above = np.indices(ln_CAV.shape)

	ln_CAV[indexes_below] = C0 + C1 * (M - 6.5) + C2 * (M - 6.5)**2 + C3 * ln_pga + C4 * ln_pga**2 + C5 * ln_pga**3 + C6 * ln_pga**4 + C7 * (np.log(vs30) - 6) + C8 * ln_dur_uni + C9 * ln_dur_uni**2
	ln_CAV[indexes_above] = C0 + C1 * (M - 6.5) + C2 * (M - 6.5)**2 + C3 * ln_pga + C7 * (np.log(vs30) - 6) + C8 * ln_dur_uni + C9 * ln_dur_uni**2
...
def calc_ln_dur_uni(pga, M, vs30):
	...
	Parameters:
		...
		M: moment magnitude
		vs30: shear-wave velocity over the top 30 m in m/s
		one or more parameters may be arrays, but if more than one parameter
		is an array, they must have the same length
	Return value:
		tuple of (ln_dur_uni, sigma_ln_dur_uni)
		- ln_dur_uni: float or array
		- sigma_ln_dur_uni: always float
	"""
	## Correlation coefficients
	a1 = 3.50 #(0.05)
	a2 = 0.0714 #(0.0421)
	...
	ln_pga = np.log(pga)
	ln_dur_uni = a1 + a2 * ln_pga + a3 / (ln_pga + a4) + a5 * (M - 6.5) + a6 * (M - 6.5)**2 + a7 * (np.log(vs30) - 6)
...
def calc_ln_CAV1(pga, M, vs30):
	...
	## Calculate ln(CAV)
	ln_pga = np.log(pga)
	ln_CAV = d1 + d2 * (ln_pga + 2.5) + d3 / (ln_pga + d4) + d5 * (M - 6.5) + d6 * (M - 6.5)**2 + d7 * (np.log(vs30) - 6)
...
def calc_CAV_exceedance_prob(pga, M, vs30, CAVmin=0.16, duration_dependent=True):
	"""
	...
	Parameters:
		...
		M: moment magnitude
		vs30: shear-wave velocity over the top 30 m in m/s
		CAVmin: CAV threshold (default: 0.16 g.s)
		duration_dependent: boolean indicating whether the two-step, duration-dependent
			or the one-step, duration-independent approach should be used (default: True)
			Note: the one-step approach is not recommended, as it could not be validated
			against the graphs shown in Abrahamson et al. (2006)
	Return value:
...
def calc_SA_exceedance_prob_given_PGA(z, pga, M, R, T, vs30, gmpe_name, correlation_model="EUS"):
	"""
	...
	"""
	ln_sa_given_pga, sigma_ln_sa_given_pga = calc_ln_SA_given_PGA(pga, M, R, T, vs30, gmpe_name, correlation_model)
...
def CAV_filtering_from_deagg_PGA(deagg_exceedances, magnitudes, intensities, vs30, CAVmin=0.16, site_amp=1):
	"""
	Perform CAV filtering based on deaggregated mean exceedance rates for PGA
	Parameters:
		deagg_exceedances: 3-D array [l,r,m] of M,r-deaggregated exceedance rates
			for a range of intensities (l)
		magnitudes: magnitude values
		intensities: intensity values
		(Note that distance values are not needed, as CAV is distance-independent,
		at least for PGA)
		vs30: shear-wave velocity over the top 30 m in m/s
		site_amp: site amplification factor (default: 1)
		CAVmin: CAV threshold (default: 0.16 g.s)
	Return value:
		tuple of (CAV_exceedances, CAV_intensities)
		CAV_intensities is identical to the input intensities, minus the last element

	Note that the CAV model was developed for surface ground motions (including site
	effects). When calculating the probability of CAV > 0.16 g.s, it is necessary to
	first scale zk(rock) to zk(soil) using the site amplification factor) !

	Note also that this can only be used for PGA at this moment.
	For spectral accelerations, a model of the relation between PGA and spectral acceleration
	needs to be used.
	"""
	...
	for k in range(len(intensities)):
		...
		CAV_exceedance_probs[k] = calc_CAV_exceedance_prob(zk, magnitude_bin_centers, vs30, CAVmin)
	...
	deagg_occurrences = np.append(deagg_occurrences, deagg_exceedances[-1:,:,:], axis=0)
	...
	CAV_deagg_exceedances = np.add.accumulate(CAV_deagg_occurrences[::-1,:,:], axis=0)[::-1,:,:]
	...
	for n in range(len(intensities)):
		...
		#for m in range(len(magnitudes)):
			...
			#for k in range(n, len(CAV_exceedances)):
				...
				#zk = intensities[k]
				#CAV_exceedances[n] += occurrence_rates[k,m] * calc_CAV_exceedance_prob(zk, M, vs30, CAVmin)

	return (CAV_total_exceedances, CAV_deagg_exceedances)


def CAV_filtering_from_deagg_SA(deagg_exceedances, magnitudes, distances, intensities, periods, gmpe_name, vs30, CAVmin=0.16):
	"""
	Perform CAV filtering based on deaggregated mean exceedance rates for PGA
	Parameters:
		deagg_exceedances: 4-D array [t,l,r,m] of M,r-deaggregated exceedance rates
			for a range of periods (t) and intensities (l)
		magnitudes: magnitude values
		distances: distance values
		intensities: 2-D array [t,l] of fixed intensity values
		periods: structural periods (s)
		gmpe_name: name of GMPE
		vs30: shear-wave velocity over the top 30 m in m/s
		CAVmin: CAV threshold (default: 0.16 g.s)
	Return value:
		tuple of (CAV_exceedances, CAV_intensities)
	...
	for t in range(len(periods)):
		...
		for k in range(num_intensities):
			...
			for r in range(len(distance_bin_centers)):
				...
				pga_values = np.exp(calc_ln_PGA_given_SA(sa, magnitude_bin_centers, R, T, vs30, gmpe_name)[0])
				pga_given_sa[t,k,r] = pga_values

	## Calculate CAV exceedance probabilities corresponding to PGA
	CAV_exceedance_probs = np.zeros((len(periods), num_intensities, len(distances), len(magnitudes)), 'd')
	for t in range(len(periods)):
		for k in range(num_intensities):
			for r in range(len(distance_bin_centers)):
				#for m in range(len(magnitude_bin_centers)):
					...
					#CAV_exceedance_probs[t,k,r,m] = calc_CAV_exceedance_prob(zk, M, vs30, CAVmin)
				zk = pga_given_sa[t,k,r]
				CAV_exceedance_probs[t,k,r] = calc_CAV_exceedance_prob(zk, magnitude_bin_centers, vs30, CAVmin)
	...
	CAV_deagg_exceedances = np.add.accumulate(CAV_deagg_occurrences[:,::-1,:,:], axis=1)[:,::-1,:,:]
	...
	cav_total_exceedances = np.add.accumulate(cav_total_occurrences[:,::-1], axis=1)[:,::-1]
	"""
...
if __name__ == "__main__":
	...
	for pga, color in zip(PGA, colors):
		CAV_exceedance_probs = calc_CAV_exceedance_prob(pga, magnitudes, vs30, CAVmin)
	...
	#	for pga, color in zip(PGA, colors):
	#		CAV_exceedance_probs = calc_CAV_exceedance_prob(pga, magnitudes, vs30, CAVmin, duration_dependent=durdep)
	...
	pylab.ylabel("Probability of CAV > %s g.s (for Vs30=%s m/s)" % (CAVmin, vs30))
	pylab.grid(True)
	pylab.legend(loc=4)
	...
	## Fig. 2-20
	"""
	from attenuation import logrange
